  ...
  return (
    <div className="min-h-screen bg-[#17181C] flex items-center justify-center p-4">
      <Card className="w-full max-w-md bg-[#2C2C2E] border-[#3A3A3C]">
        <CardHeader>
          <CardTitle className="text-center text-white">🚗 CarWash Admin</CardTitle>
          <p className="text-center text-sm text-[#CCCCCC]">Панель администратора</p>
        </CardHeader>
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="space-y-2">
              <label className="text-sm font-medium text-white">Номер телефона</label>
              <Input
                type="tel"
                value={phone}
            ...
            {forbiddenMessage && (
              <div className="rounded-md border border-red-500/50 bg-red-500/10 p-3 text-sm text-red-200" role="alert">
                {forbiddenMessage}
              </div>
            )}
            {error && (
